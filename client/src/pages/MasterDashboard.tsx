        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-4xl font-bold bg-gradient-to-r from-cyan-400 to-purple-500 bg-clip-text text-transparent">
              👑 الروبوت القائد (Master)
            </h1>
            <p className="text-muted mt-1">✨ نظام التحكم والمراقبة الرئيسي</p>
          </div>
          <Button
            onClick={() => setLocation('/')}
            variant="outline"
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-1 p-6 bg-card border-border">
            <h2 className="text-xl font-bold mb-6 text-cyan-400">التحكم بالمحرك</h2>
          ...
          <Card className="lg:col-span-2 p-6 bg-card border-border">
            <h2 className="text-xl font-bold mb-6 text-cyan-400">إحصائيات الطاقة</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="p-6 bg-card border-border">
            <h3 className="text-lg font-bold mb-4 text-cyan-400">استهلاك الطاقة</h3>
          ...
          <Card className="p-6 bg-card border-border">
            <h3 className="text-lg font-bold mb-4 text-cyan-400">الجهد والتيار</h3>
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={chartData}>
                ...
                <Tooltip contentStyle={{ backgroundColor: '#1A1F3A', border: '1px solid #2D3E5F' }} />
                <Legend />
                <Line type="monotone" dataKey="voltage" stroke="#00D9FF" dot={false} strokeWidth={2} name="الجهد" />
                <Line type="monotone" dataKey="current" stroke="#B026FF" dot={false} strokeWidth={2} name="التيار" />
              </LineChart>
            </ResponsiveContainer>
          </Card>
        </div>
        ...
        <Card className="p-6 bg-card border-border">
          <h3 className="text-lg font-bold mb-4 text-cyan-400">مراقبة درجة الحرارة</h3>
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={chartData}>
              <defs>
              ...
              <YAxis stroke="#A0A0A0" />
              <Tooltip contentStyle={{ backgroundColor: '#1A1F3A', border: '1px solid #2D3E5F' }} />
              <Area type="monotone" dataKey="temperature" stroke="#FF6B35" fillOpacity={1} fill="url(#colorTemp)" name="درجة الحرارة" />
            </AreaChart>
          </ResponsiveContainer>
        </Card>

        <div className="p-4 bg-card border border-border rounded-lg text-center text-sm text-muted">
          <p>🚀 لوحة تحكم الروبوت القائد - بيانات فعلية من ESP32</p>
        </div>
      </div>
    </div>
  );
