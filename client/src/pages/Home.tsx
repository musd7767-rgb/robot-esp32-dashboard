...
import { Power, Zap, Gauge, AlertCircle, Play, Square, RotateCw } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card className="p-6 bg-card border-cyan-500/30">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-muted text-sm font-medium">Voltage</p>
          ...
          <Card className="p-6 bg-card border-purple-500/30">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-muted text-sm font-medium">Current</p>
          ...
          <Card className="p-6 bg-card border-green-500/30">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-muted text-sm font-medium">Power</p>
          ...
          <Card className="p-6 bg-card border-orange-500/30">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-muted text-sm font-medium">Temperature</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-1 p-6 bg-card border-border">
            <h2 className="text-xl font-bold mb-6 text-cyan-400">Motor Control</h2>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3 pt-4">
                <Button onClick={() => handleMotorCommand('forward')} className="w-full h-12 bg-green-600 hover:bg-green-700 text-white">
                  <Play className="w-4 h-4 mr-2" /> Forward
                </Button>
                ...
                <Button onClick={() => handleMotorCommand('backward')} className="w-full h-12 bg-blue-600 hover:bg-blue-700 text-white">
                  <RotateCw className="w-4 h-4 mr-2" /> Back
                </Button>
              </div>

              <div className="mt-6 p-4 bg-background rounded-lg border border-border">
                <p className="text-xs text-muted mb-1">Motor Status</p>
                <p className="text-lg font-bold text-cyan-400 capitalize">{robotData.motorStatus}</p>
              </div>
            </div>
          </Card>

          <Card className="lg:col-span-2 p-6 bg-card border-border">
            <h2 className="text-xl font-bold mb-6 text-cyan-400">Power Statistics</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="p-4 bg-background rounded-lg border border-border">
                <p className="text-xs text-muted mb-2">Max Power</p>
                <p className="text-2xl font-bold text-green-400">{stats.maxPower.toFixed(2)}W</p>
              </div>
              <div className="p-4 bg-background rounded-lg border border-border">
                <p className="text-xs text-muted mb-2">Avg Power</p>
                <p className="text-2xl font-bold text-cyan-400">{stats.avgPower.toFixed(2)}W</p>
              </div>
              <div className="p-4 bg-background rounded-lg border border-border">
                <p className="text-xs text-muted mb-2">Max Current</p>
                <p className="text-2xl font-bold text-purple-400">{stats.maxCurrent.toFixed(2)}A</p>
              </div>
              <div className="p-4 bg-background rounded-lg border border-border">
                <p className="text-xs text-muted mb-2">Total Energy</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="p-6 bg-card border-border">
            <h3 className="text-lg font-bold mb-4 text-cyan-400">Power Consumption</h3>
          ...
          <Card className="p-6 bg-card border-border">
            <h3 className="text-lg font-bold mb-4 text-cyan-400">Voltage and Current</h3>
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={chartData}>
                ...
                <Tooltip contentStyle={{ backgroundColor: '#1A1F3A', border: '1px solid #2D3E5F' }} />
                <Legend />
                <Line type="monotone" dataKey="voltage" stroke="#00D9FF" dot={false} strokeWidth={2} />
                <Line type="monotone" dataKey="current" stroke="#B026FF" dot={false} strokeWidth={2} />
              </LineChart>
            </ResponsiveContainer>
          </Card>
        </div>
        ...
        <Card className="p-6 bg-card border-border">
          <h3 className="text-lg font-bold mb-4 text-cyan-400">Temperature Monitoring</h3>
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={chartData}>
              <defs>
              ...
              <YAxis stroke="#A0A0A0" />
              <Tooltip contentStyle={{ backgroundColor: '#1A1F3A', border: '1px solid #2D3E5F' }} />
              <Area type="monotone" dataKey="temperature" stroke="#FF6B35" fillOpacity={1} fill="url(#colorTemp)" />
            </AreaChart>
          </ResponsiveContainer>
        </Card>
